...
Dokumentacja HTML do SAP DB. Więcej informacji można znaleźć na
...
Ten pakiet zawiera skrypt tp tworzący instancję TST bazy danych
(zawierającą 20 MB miejsca na dane i 8 MB miejsca na logi) i
natychmiast ją uruchamiający. Należy pamiętać o wyłączeniu bazy
danych przed zamknięciem systemu.
- przykładowy użytkownik bazy danych: TEST, hasło TEST
- użytkownik DBM: DBM, hasło DBM
- administrator bazy danych: DBA, hasło DBA
Znajduje się tu również skrypt usuwający tę bazę danych.
Skrypty znajdują się w katalogu %{sapdbdir}/testdb i powinien je
uruchamiać użytkownik sapdb.
